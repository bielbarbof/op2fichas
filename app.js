...
const parentModalId = params.get('modalId') || 'com.op2.playtest.chat/open-fichas';
...
async function openSheet(id){await OBR.modal.open({id:SHEET_MODAL_ID,url:`/sheet.html?id=${encodeURIComponent(id)}${openedFromChat?'&from=chat':''}`,fullScreen:true})}
async function closeToChat(){if(!OBR.isAvailable){history.back();return}try{await OBR.modal.close(parentModalId)}catch{history.back()}}
...
function renderGM(){
  const cards=Object.values(CHARACTERS).map(c=>{
    ...
    return `<article class="player-sheet-card gm-preview-card" style="--accent:${c.accent}">
      <div class="player-top"><div class="player-portrait"><img src="${c.token}" alt="${escapeHtml(c.name)}" /></div><div class="player-info"><div class="player-identity"><div class="player-name-row"><h2>${escapeHtml(c.name)}</h2></div><div class="bigmeta"><span class="player-profile">${escapeHtml(c.profile)}</span><span class="meta-dot">•</span><span class="player-occupation">${escapeHtml(c.occupation)}</span><span class="meta-dot">•</span>${levelBadge(c.level)}</div></div><div class="stats">${attrs}</div></div></div>
...
async function setup(){
  await loadVersion();
  closeChatModal.classList.toggle('hidden',!openedFromChat);closeChatModal.addEventListener('click',closeToChat);
  if(!OBR.isAvailable){roleBadge.textContent='PRÉVIA';state.role='GM';renderGM();return}
  await new Promise(resolve=>OBR.onReady(resolve));[state.role,state.party]=await Promise.all([OBR.player.getRole(),OBR.party.getPlayers()]);state.playerId=OBR.player.id;await loadRoomState();if(state.role==='GM'){const meta=await OBR.room.getMetadata();if(!meta[ROOM_STATE_KEY])await saveRoomState(state.roomState)}render();OBR.room.onMetadataChange(meta=>{if(meta[ROOM_STATE_KEY]){state.roomState=normalizeRuntimeState(meta[ROOM_STATE_KEY]);render()}});OBR.party.onChange(p=>{state.party=p;render()})
}
