...
function renderGM(){
  const cards=Object.values(CHARACTERS).map(c=>{
    ...
    return `<article class="player-sheet-card gm-preview-card" style="--accent:${c.accent}">
      <div class="player-top"><div class="player-portrait"><img src="${c.token}" alt="${escapeHtml(c.name)}" /></div><div class="player-info"><div class="player-name-row"><h2>${escapeHtml(c.name)}</h2></div><div class="bigmeta"><span class="player-profile">${escapeHtml(c.profile)}</span><span class="meta-dot">•</span><span class="player-occupation">${escapeHtml(c.occupation)}</span><span class="meta-dot">•</span>${levelBadge(c.level)}</div><div class="stats">${attrs}</div></div></div>
