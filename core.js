...
export function normalizeRuntimeState(raw) {
  ...
  for (const [id, character] of Object.entries(CHARACTERS)) {
    ...
    target.pendingDice = Array.isArray(incoming.pendingDice)
      ? incoming.pendingDice.slice(-4).map(x => ({ id:String(x.id || crypto.randomUUID?.() || Date.now()), sides:4, scope:String(x.scope || 'any'), source:String(x.source || 'Bônus') }))
      : [];
...
export function encodeSize(value) {
  return new TextEncoder().encode(JSON.stringify(value)).length;
}

export function validateStateSize(state) {
  const bytes = encodeSize(state);
  return { bytes, ok: bytes < 14500 };
}

export function assignedCharacterId(state, playerId) {
  return Object.keys(CHARACTERS).find(id => state.assignments?.[id]?.playerId === playerId) || null;
}

export function isAuthorized(state, characterId, playerId, role) {
  return role === 'GM' || state.assignments?.[characterId]?.playerId === playerId;
}

export function makeBonusDie(source, scope='any') {
  return { id: crypto.randomUUID?.() || `${Date.now()}-${Math.random()}`, sides:4, scope, source };
}
...
export function applyOperation(stateInput, operation, sender) {
  ...
  switch (operation.type) {
    ...
    case 'adjust-resource':
      ...
      break;
    case 'impulse-set':
      runtime.impulse = clamp(Math.trunc(operation.value), 0, 3);
      break;
    case 'impulse-adjust':
      runtime.impulse = clamp(runtime.impulse + Math.trunc(operation.delta || 0), 0, 3);
      break;
    case 'queue-die':
      runtime.pendingDice = [...runtime.pendingDice, makeBonusDie(String(operation.source || 'Bônus'), String(operation.scope || 'any'))].slice(-4);
      break;
    ...
    case 'evaluation-set':
      runtime.evaluationDice = clamp(Math.trunc(operation.value), 0, 2);
      break;
    case 'evaluation-use': {
      ...
      for (let i=0;i<count;i++) runtime.pendingDice.push(makeBonusDie('Avaliação', 'any'));
    ...
    case 'step-reset':
      runtime.stepMods = { fisico:0, mente:0, emocao:0 };
      break;
    case 'ability-focus': {
      const cost = 2;
      if (runtime.pd < cost) throw new Error('PD insuficiente.');
      runtime.pd -= cost;
      runtime.pendingDice = [...runtime.pendingDice, makeBonusDie(String(operation.source || 'Foco'), String(operation.scope || 'any'))].slice(-4);
      break;
    }
    ...
    case 'impulse-spend-one':
      ...
      runtime.impulse -= 1;
      runtime.pendingDice = [...runtime.pendingDice, makeBonusDie('Ímpeto', 'any')].slice(-4);
      break;
    ...
    case 'readiness-set':
      runtime.readiness = Boolean(operation.value);
      break;
    default:
